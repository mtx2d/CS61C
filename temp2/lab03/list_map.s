...
map:
    ...
    # THINK: why a0?
    ### YOUR CODE HERE ###
    lw a0, 0(s0) # a0 (argument) contains current node value

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why).
    # What function? Recall the parameters of "map"
    ### YOUR CODE HERE ###
    jalr ra, s1, 0 # go to next instruction AFTER executing square()
                   # We can't use a label b/c... 
    
    # store the returned value back into the node
    # Where can you assume the returned value is?
    ### YOUR CODE HERE ###
    sw a0, 0(s0) # s0 is a POINTER to a0--it changes what a0 holds: nodeValue --> square() result

    # Load the address of the next node into a0
    # The Address of the next node is an attribute of the current node.
    # Think about how structs are organized in memory.
    ### YOUR CODE HERE ###
    lw a0, 4(s0) # s0 is a POINTER to a0--so a0 now contains the next value in the linked list

    # Put the address of the function back into a1 to prepare for the recursion
    # THINK: why a1? What about a0?
    ### YOUR CODE HERE ###
    addi a1, s1, 0 # a1 is used b/c a0 is used to hold the function arguments (can't also contain function name)

    # recurse
    ### YOUR CODE HERE ###
    jal ra, map # Go back to next instruction after calling map()
