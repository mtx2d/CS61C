...
mapLoop:
    lw t1, 0(s0)		# ERROR 1 (Previous: add t1, s0, x0): should be using lw instead of add to get pointer to array
    lw t2, 4(s0)        # load the size of the node's array into t2
	slli t3, t0, 2      # ERROR 2: The actual offset needs to account for 4x (each word = 4 bytes)
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

	addi sp, sp, -20 # ERROR 3: lines 75-85: storing & retrieving temps & ra
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    sw t3, 12(sp)
	...
	lw a0, 8(s0) # ERROR 4: (Previous: la a0, 8(s0))--Load the next node (previous version obtains address of the address)
    add a1, s1, x0 # ERROR 5: (Previous: lw a1, 0(s1))--Load function address--We want the address of the function (already stored in s1)
    jal  map            # recurseThe 
done:
    lw s0, 8(sp)
    lw s1, 4(sp)
